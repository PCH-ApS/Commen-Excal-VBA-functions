Attribute VB_Name = "SourceSearchStringFoundInTarget"
Option Explicit
'@Folder "Code Common"
Public Function SourceSearchStringFoundInTargetArrayV2(ByVal sourceSearchString As String, ByRef targetArray As Variant, ByRef headerArray As Variant, ByVal searchKey As Variant) As Boolean
                ...
                For headerRowCount = LBound(headerArray, 1) To UBound(headerArray, 1)
                    If Not InStr(headerArray(headerRowCount, 1), "*") > 0 Then
                        If headerArray(headerRowCount, 1) = targetArray(1, targetColumnCount) Then
                            targetCompareString = targetCompareString & ";" & Trim$(CStr(targetArray(targetRowCount, targetColumnCount)))
                            Exit For
                        End If
                    End If
                Next headerRowCount
